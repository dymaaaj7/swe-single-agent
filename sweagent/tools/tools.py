...
from sweagent.tools.parsing import FunctionCallingParser, ParseFunction
...
class ToolConfig(BaseModel):
    ...
    """Directory where tool bundles will be uploaded/installed.
    Default is `/root/tools` for Docker deployments.
    For local deployments, change this to a writable directory like `/tmp/swe-agent-tools`.
    """
...
class ToolHandler:
    ...
    def reset(self, env: SWEEnv) -> None:
        ...
        env.write_file(f"{tools_dir}/.swe-agent-env", json.dumps(self.config.registry_variables))
        env.write_file(f"{tools_dir}/state.json", "{}")
